...
>
> **与透传节点模式联动**：执行队列时会检查画布上「文本（透传）」节点的模式——
> - 透传为「📥 输入」模式：正常回传，上游文本回填左框
> - 透传为「📝 内容」模式：**不回传**——左框中你互换过来的原文不会被覆盖
>
> 已知限制：受 ComfyUI 执行机制限制（OUTPUT_NODE + lazy 组合下执行器不会重新调度，已实测），内容模式下上游反推节点仍会随队列执行一次（数据不受影响）；如需完全跳过反推，请对其 Ctrl+B bypass。
>
