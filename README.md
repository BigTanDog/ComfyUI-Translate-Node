...
**格式契约**：透传节点文本按空行分段——第 1 段视为提示词（tags）**原样保留**，其余段整体替换为译文：

```
masterpiece, best quality, score_9, ...     ← 提示词段，永远保留
                                            ← 空行
（这里会被替换为翻译节点的译文）              ← 内容段
```

- 目标内容没有空行分段（契约不满足）时**不替换，改为追加**，状态栏会提示
- 透传节点处于「📥 输入」模式时发送会被拦截，需先切到「📝 内容」
- 配套改动：透传节点插件（`danbooru_text_passthrough.js`）已打补丁——上游回传内容写入编辑器前会经 `_ctnNormalize` 结构守护，单段上游文本不会再冲掉已有的提示词段
- 画布上有多个透传节点时会弹窗选择目标
